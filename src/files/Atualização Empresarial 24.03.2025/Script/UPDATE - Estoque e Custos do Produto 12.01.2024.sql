...
SET PRD_ESTOQUE_ENTRADAS = COALESCE((SELECT SUM(NEI.NFEI_PRDQTDEMOV)
                           FROM EMP_ENTRADAI NEI
                           WHERE NEI.NFEI_FKPRD_ID = Q.PRD_ID),0),

    PRD_ESTOQUE_SAIDAS = COALESCE((SELECT SUM(NSI.NFSI_PRD_QTD_MOV)
                         FROM EMP_NFSI NSI
                         WHERE NSI.NFSI_FKPRD_ID = Q.PRD_ID AND NSI.NFSI_LKCFOP_NOME != '59291'),0), /*ACRESCENTADO COMANDO PARA NÃO SOMAR NOTAS DE SUBSTITUIÇÃO DE CUPOM*/

    PRD_ESTOQUE_ATUAL = (COALESCE((SELECT SUM(NEI.NFEI_PRDQTDEMOV)
                         FROM EMP_ENTRADAI NEI
                         WHERE NEI.NFEI_FKPRD_ID = Q.PRD_ID),0) - COALESCE((SELECT SUM(NSI.NFSI_PRD_QTD_MOV)
                                                                  FROM EMP_NFSI NSI /*ACRESCENTADO COMANDO PARA NÃO SOMAR NOTAS DE SUBSTITUIÇÃO DE CUPOM*/
